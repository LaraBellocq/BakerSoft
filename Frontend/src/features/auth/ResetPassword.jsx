...
import { confirmPasswordReset, logPasswordEvent } from './api';
...
export default function ResetPassword() {
  ...
  const [globalError, setGlobalError] = useState(token ? '' : 'Token inválido o expirado.');
  ...
  const handleSubmit = async (event) => {
    ...
    if (!password) {
      ...
      nextClientErrors.password = 'Debe cumplir la política de seguridad';
    }

    if (!confirmPassword) {
      nextClientErrors.confirmPassword = 'Confirma tu contraseña';
    } else if (password && password !== confirmPassword) {
      nextClientErrors.confirmPassword = 'Las contraseñas no coinciden';
    }

    if (!token) {
      setGlobalError('Token inválido o expirado.');
      setTokenIssue(true);
    }
    ...
    try {
      ...
      const message = tokenMessage || globalMessage || 'Ocurrió un error. Inténtalo nuevamente.';
      setGlobalError(message);
      setTokenIssue(Boolean(tokenMessage) || message.toLowerCase().includes('token'));
      await logPasswordEvent({ email: '', action: 'confirm', success: false });
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="form-card">
        ...
        {successMessage ? (
          <div className="alert success" role="status" aria-live="polite">
            <div className="alert-message"><span>{successMessage}</span></div>
            <Link className="secondary full-width" to="/login">Ir a iniciar sesión</Link>
          </div>
        ) : null}
        ...
        {globalError && !successMessage ? (
          <div className="alert error" role="alert" aria-live="assertive">
            <p>{globalError}</p>
            {tokenIssue ? (
              <Link className="secondary full-width" to="/forgot-password">Volver a solicitar enlace</Link>
            ) : null}
          </div>
        ) : null}
        ...
        <form onSubmit={handleSubmit} noValidate>
          <div className="field">
            ...
            />
            <p id="reset-password-hint" className="hint">Debe cumplir la política de seguridad.</p>
            {passwordError ? <p id="reset-password-error" className="error" role="alert">{passwordError}</p> : null}
          </div>

          <div className="field">
            <label htmlFor="reset-confirm-password">Confirmar contraseña</label>
            <input
              id="reset-confirm-password" type="password" autoComplete="new-password"
              value={confirmPassword} onChange={(e)=>setConfirmPassword(e.target.value)}
              aria-required="true" aria-invalid={Boolean(confirmError)}
              aria-describedby={confirmError ? 'reset-confirm-password-error' : undefined}
              className={confirmError ? 'has-error' : ''} disabled={loading} required
            />
            {confirmError ? <p id="reset-confirm-password-error" className="error" role="alert">{confirmError}</p> : null}
          </div>
